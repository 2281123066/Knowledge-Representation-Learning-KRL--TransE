...
class TransE:
    ...
    def transE(self, cI = 20):
        ...
        for cycleIndex in range(cI):
            ...
            self.update(Tbatch)#得到更新之后的头、尾实体向量
            if cycleIndex % 100 == 0:
                print("第%d次循环" % cycleIndex)
                print(self.loss)
                self.writeRelationVector("E:\\workplace_kdh\\KG\\transE\\train_result\\relationVector.txt")
                self.writeEntilyVector("E:\\workplace_kdh\\KG\\transE\\train_result\\entityVector.txt")
                self.loss = 0
...
def openTrain(dir,sp="\t"):
    ...
    with open(dir) as file:
        ...
        for line in lines:
            ...
            if(len(triple)<3):
                continue
            list.append(tuple(triple))#将训练数据转换成元组形式（ ， ， ）
            num += 1
    return num, list

if __name__ == '__main__':
    dirEntity = "E:\\workplace_kdh\\KG\data\\WN18\\entity2id.txt"
    entityIdNum, entityList = openDetailsAndId(dirEntity)
    dirRelation = "E:\\workplace_kdh\\KG\\data\\WN18\\relation2id.txt"
    ...
    print("TranE初始化完成")
    transE.transE(15000)
    transE.writeRelationVector("E:\\workplace_kdh\\KG\\transE\\train_result\\relationVector.txt")
    transE.writeEntilyVector("E:\\workplace_kdh\\KG\\transE\\train_result\\entityVector.txt")
